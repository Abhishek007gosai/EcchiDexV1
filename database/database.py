...
catalog_cache_col = _db["catalog_cache"]
...
# ---------------------------------------------------------------------------
# Catalog cache — a persisted mirror of AniListSource's in-memory discover
# cache (trending/popular/genre pages). The in-memory cache is faster but
# dies on every restart; every restart on a free-tier host (Render/Koyeb
# spinning down an idle instance, or any redeploy) previously meant the
# first visitor paid for live, rate-limited AniList calls. Persisting the
# last known-good response here means a fresh process can serve it
# instantly instead, while refreshing in the background. See
# plugins/anilist.py's _cached().
# ---------------------------------------------------------------------------

def get_catalog_cache(key: str) -> dict | None:
    doc = catalog_cache_col.find_one({"_id": key})
    if not doc:
        return None
    return {"value": doc["value"], "cached_at": doc["cached_at"]}


def set_catalog_cache(key: str, value: dict) -> None:
